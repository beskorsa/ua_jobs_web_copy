...
- OCR для сканованих PDF-резюме (tesseract.js, фоллбек коли `pdf-parse` не
  знайшов текстового шару) є тільки в веб-частині (`src/lib/ocr.ts`). У
  Python-парсері (`ua_jobs_parser/resume_parser.py`) OCR все ще не
  підтримується — там PDF читається тільки через PyMuPDF, для скан-копій
  без текстового шару потрібне ручне розпізнавання.
