...
import { touchKeyword, seedKeywordDictionary } from "@/lib/keywords";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const results = await semanticSearch(vec, topK, minusKeywords);
    ...
    // додається, відоме отримує +1 до популярності.
    await Promise.all([
